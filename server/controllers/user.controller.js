...
const Roles = ["System Admin", "STS Manager", "Landfill Manager"];
...
async function fetchAvailableRoles(_req, res) {
  try {
    ...
    return res
      .status(500)
      .send({ message: "Fetch available roles failed with error!" });
  }
}
...
async function fetchUserRoles(req, res) {
  ...
  try {
    ...
    return res
      .status(500)
      .send({ message: "Fetch user roles failed with error!" });
  }
}
